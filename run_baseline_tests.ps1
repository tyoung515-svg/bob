# BoBClaw release-surface baseline test pass.
#
# Runs every release suite (core / gateway / pipeline / TUI pytest, plus the KMM
# shared JVM tests and desktop Kotlin compile) and a dependency check. Writes one
# log per suite under _workspace/testing/v098/ and EXITS NON-ZERO if any suite
# fails — a green print with a red suite underneath was the P1 "false green" bug.
#
...
$logdir = Join-Path $repo "_workspace\testing\v098"
...
Invoke-PySuite -Name "tui"      -Dir "bobclaw-tui"             -PyPath "."
