...
| `bobclaw-gateway` | Python / aiohttp | 7826 | JWT + TOTP auth, WebSocket chat, REST (conversations / messages / faces / models / teams / profiles / approvals / memory). JSON + WS only — the desktop app is the GUI. The only client-facing service. |
| `bobclaw-claude-pipeline` | Python | 7823 | Claude build-session wrapper (holds an Anthropic key). |
| `bobclaw-app` | Kotlin Multiplatform | — | The GUI client — desktop; Android preview. Localized EN / 简 / 繁. |
