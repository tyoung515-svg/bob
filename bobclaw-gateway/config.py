...
class BoBClawGatewayConfig:
    ...
    # Safer default: bind loopback only. Matches the release repo (bob) default.
    # Override with BOBCLAW_GATEWAY_HOST=0.0.0.0 to expose on all interfaces.
    HOST: str = os.getenv("BOBCLAW_GATEWAY_HOST", "127.0.0.1")

    # Trust the reverse proxy's X-Forwarded-For for client-IP resolution (the
    # rate-limit / audit-log key). OFF by default (loopback / direct connection —
    # request.remote is the real client). Enable ONLY behind EXACTLY ONE trusted
    # proxy — see client_ip.py; the rightmost XFF entry is used, so a client cannot
    # spoof it to evade the rate limiter.
    ...
    # -- Streaming concurrency (per-user cap on simultaneously live chat streams) --
    MAX_CONCURRENT_STREAMS_PER_USER: int = int(os.getenv("BOBCLAW_MAX_CONCURRENT_STREAMS_PER_USER", "4"))
