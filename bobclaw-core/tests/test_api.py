...
# ─── R0: GPT-5.6 roster visible on the faces surface ──────────────────────────

async def test_faces_surface_exposes_gpt56_roster(client):
    ...
    expected_models = {
        "planner-gpt56-sol": "gpt-5.6-sol",
        "worker-gpt56-terra": "gpt-5.6-terra",
        "worker-gpt56-luna": "gpt-5.6-luna",
    }
    for face_id, expected_model in expected_models.items():
        response = await client.get(f"/api/faces/{face_id}")
        assert response.status == 200
        face = await response.json()
        assert face["codex_posture"]["model"] == expected_model
