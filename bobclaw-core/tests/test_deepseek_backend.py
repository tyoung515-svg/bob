"""
BoBClaw Core — Unit tests for DeepSeekClient
...
def _make_client(api_key: str = "dummy-key", base_url: str | None = None) -> DeepSeekClient:
...
def test_custom_base_url_override():
    client = _make_client(base_url="https://custom.deepseek.com/v1")
...
async def test_chat_posts_to_chat_completions_with_bearer():
    ...
    with patch("aiohttp.ClientSession.post", side_effect=fake_post):
        result = await client.chat(messages=[{"role": "user", "content": "hi"}], model="deepseek-chat")
